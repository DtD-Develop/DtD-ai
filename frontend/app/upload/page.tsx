...
export default function UploadKbPage() {
  ...
  // โหลดไฟล์ครั้งแรก + polling ทุก 5 วิ (รายการทั้งหมด)
  useEffect(() => {
  ...
  const statusColor = (status: KbFile["status"]) => {
    switch (status) {
      case "uploaded":
      case "parsing":
        return "bg-sky-100 text-sky-800 border-sky-300 dark:bg-sky-500/20 dark:text-sky-200 dark:border-sky-500/40";
      case "tagged":
      ...
      case "ready":
        return "bg-emerald-100 text-emerald-800 border-emerald-300 dark:bg-emerald-500/20 dark:text-emerald-200 dark:border-emerald-500/40";
      case "failed":
        return "bg-rose-100 text-rose-800 border-rose-300 dark:bg-rose-500/20 dark:text-rose-200 dark:border-rose-500/40";
      default:
        return "bg-slate-100 text-slate-700 border-slate-300 dark:bg-slate-500/20 dark:text-slate-200 dark:border-slate-500/40";
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Message */}
        {message && (
          <div className="mb-4 rounded-lg border border-slate-200 bg-slate-100 px-3 py-2 text-xs text-slate-800 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100">
            {message}
          </div>
        )}
        ...
        <div className="mb-6">
          <div
            className="border border-dashed border-slate-300 bg-white dark:border-slate-600 dark:bg-slate-900/60 rounded-xl p-6 flex flex-col md:flex-row items-center justify-between gap-4"
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <label className="inline-flex cursor-pointer items-center rounded-full bg-sky-600 hover:bg-sky-500 px-4 py-2 text-xs font-medium text-white shadow-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Table */}
          <section className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900/60 p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm font-semibold">KB Files</div>
              {loading && (
                <div className="text-[10px] text-slate-500 dark:text-slate-400 animate-pulse">
                  Refreshing…
                </div>
              )}
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead className="text-[10px] uppercase text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                  <tr>
                ...
                <tbody>
                  ...
                  {files.map((file) => (
                    <tr
                      key={file.id}
                      className={`border-b border-slate-100 dark:border-slate-800/60 hover:bg-slate-50 dark:hover:bg-slate-800/60 cursor-pointer ${
                    ...
                    >
                      ...
                      <td className="py-2 pr-2 align-top">
                        <div className="flex flex-wrap gap-1 max-w-[220px]">
                          {/* confirmed tags = ฟ้า, auto tags = เทา */}
                          {file.tags &&
                            file.tags.map((t) => (
                              <span
                                key={`tag-${t}`}
                                className="inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium bg-sky-100 text-sky-800 border-sky-300 dark:bg-sky-500/20 dark:text-sky-100 dark:border-sky-500/40"
                              >
                                {t}
                              </span>
                            ))}
                          ...
                          {!file.tags?.length &&
                            (file.auto_tags || []).map((t) => (
                              <span
                                key={`auto-${t}`}
                                className="inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] text-[10px] bg-slate-100 text-slate-700 border-slate-200 dark:bg-slate-800 dark:text-slate-200 dark:border-slate-600"
                              >
                                {t}
                              </span>
                            ))}

                          {!file.auto_tags?.length && !file.tags?.length && (
                      ...
                      <td className="py-2 pl-2 align-top text-right space-x-1">
                        <button
                          ...
                          className="inline-flex items-center rounded-full border border-slate-300 dark:border-slate-600 px-2 py-0.5 text-[10px] hover:bg-slate-100 dark:hover:bg-slate-700"
                        >
                          Edit tags
                        </button>
                        <button
                          ...
                          className="inline-flex items-center rounded-full border border-emerald-500 px-2 py-0.5 text-[10px] text-emerald-600 dark:text-emerald-300 hover:bg-emerald-50 dark:hover:bg-emerald-500/20 disabled:opacity-40"
                        >
                          Confirm
                        </button>
                        <button
                          ...
                          className="inline-flex items-center rounded-full border border-rose-500 px-2 py-0.5 text-[10px] text-rose-600 dark:text-rose-300 hover:bg-rose-50 dark:hover:bg-rose-500/20"
                        >
                          Delete
                        </button>
                      </td>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900/60 p-4 flex flex-col">
              ...
              <>
                ...
                <div className="mb-4">
                  <button
                    ...
                    className="rounded-full bg-sky-600 hover:bg-sky-500 px-3 py-1 text-[11px] font-medium text-white shadow-sm"
                  >
                    Save tags
                  </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto border border-slate-200 dark:border-slate-800 rounded-md bg-slate-50 dark:bg-slate-950/40 p-2 space-y-1 text-[11px]">
